...
class TrayIcon:
    ...
    def _start(self):
        # Tray icon is managed globally, not per-client, but it needs a loop.
        # We assume the first client's loop or main loop handles this.
        loop = asyncio.get_running_loop()
        
        # We need a way to get the 'current' drop to display.
        # For simplicity, we can default to the first client's progress or None.
        # The GUIManager handles updates to the tray.
        drop = None # self._manager.progress._drop # This is tricky in multi-user
...
class SettingsPanel:
    ...
    def __init__(self, manager: GUIManager, master: ttk.Widget):
        self._manager = manager
        self._settings: Settings = manager.settings
...
class UserGUIAdapter:
    ...
    def __init__(self, manager: GUIManager, user_idx: int, client: Twitch):
        ...
        # Share global attributes
        self.tray = manager.tray
        self._style = manager._style  # Required by ChannelList
        self.tabs = manager.tabs
        self._cache = manager._cache
        self.settings = manager.settings # This must be the Settings object
        
        # Compatibility properties for components accessing manager attributes
        self._twitch = client
...
class GUIManager:
    def __init__(self, twitch: Twitch):
        ...
        # If it does, we are initializing a secondary Twitch client (User 2-20).
        if tk._default_root:
            self._root = tk._default_root
            # Minimal initialization for temporary instance
            self._twitch = twitch
            self.settings = twitch.settings
            return

        # --- Main GUIManager Initialization (User 1) ---
        self._root = Tk(className=WINDOW_TITLE)
        self._root.withdraw()
        set_root_icon(self._root, resource_path("icons/pickaxe.ico"))
        self._root.title(WINDOW_TITLE)
        self._root.bind_all("<KeyPress-Escape>", self.unfocus)

        # Initialize core references immediately
        self._twitch = twitch
        self.settings = twitch.settings
        self._clients: list[Twitch] = [twitch]
        ...
        self._handlers: list[logging.Handler] = [] # Track handlers to remove them later

        # Style setup
        self._style = style = ttk.Style(self._root)
        style.map("Treeview", foreground=self._fixed_map("foreground"), background=self._fixed_map("background"))
        ...
        # Root Frame
        root_frame = ttk.Frame(self._root, padding=8)
        ...
        # 1. Main Tab (will contain User Tabs)
        ...
        # 2. Inventory Tab (will contain User Tabs)
        ...
        # 3. Settings Tab (Shared)
        settings_frame = ttk.Frame(root_frame, padding=8)
        # Assign to self.settings_panel, NOT self.settings, to keep data object separate
        ...
        for i in range(20):
            ...
            # Create Client
            if i == 0:
                current_client = twitch
                self._patch_client(current_client, user_num)
            else:
                # Create new client instances for User 2-20
                current_client = client_cls(settings)
                self._patch_client(current_client, user_num)
                self._clients.append(current_client)

            # Create Adapter
            adapter = UserGUIAdapter(self, user_num, current_client)
            current_client.gui = adapter # Replace the partial GUI created in Twitch.__init__
            ...
            adapter.status = StatusBar(user_main_frame)
            adapter.websockets = WebsocketStatus(user_main_frame)
            adapter.login = LoginForm(user_main_frame, current_client)
            adapter.progress = CampaignProgress(user_main_frame)
            adapter.output = ConsoleOutput(user_main_frame)
            adapter.channels = ChannelList(user_main_frame, current_client)

            # Setup Logging
            h = _TKOutputHandler(adapter.output)
            h.setFormatter(OUTPUT_FORMATTER)
            logging.getLogger("TwitchDrops").addHandler(h)
        ...
        if sys.platform == "win32":
            ...
            self._root.protocol("WM_DESTROY_WINDOW", self.close)
        ...
        self.apply_theme(twitch.settings.dark_mode)

        if twitch.settings.tray:
            self._root.after_idle(self.tray.minimize)
        else:
            self._root.after_idle(self._root.deiconify)

    def __getattr__(self, name: str):
        # Proxy attribute access to User 1's adapter.
        # This allows the original client (User 1) to access 'self.gui.channels' etc.
        # even though 'self.gui' is the GUIManager instance.
        if self.adapters:
            return getattr(self.adapters[0], name)
        raise AttributeError(f"'{type(self).__name__}' object has no attribute '{name}'")

    def _patch_client(self, client: Twitch, user_idx: int):
        """
        Dynamically patch the Twitch client instance to use a separate cookie file.
        """
        # Patch get_session
        async def new_get_session() -> aiohttp.ClientSession:
            if (session := client._session) is not None:
                if session.closed:
                    raise RuntimeError("Session is closed")
                return session
            
            cookie_jar = aiohttp.CookieJar()
            cookie_path = Path(WORKING_DIR, f"cookies_user_{user_idx}.jar")
            try:
            ...
            connection_quality = client.settings.connection_quality
            if connection_quality < 1:
                connection_quality = client.settings.connection_quality = 1
            elif connection_quality > 6:
                connection_quality = client.settings.connection_quality = 6
            ...
            client._session = aiohttp.ClientSession(
                ...
                headers={"User-Agent": client._client_type.USER_AGENT},
            )
            return client._session

        client.get_session = new_get_session

        # Patch shutdown to save to the correct file
        async def new_shutdown() -> None:
            start_time = time()
            client.stop_watching()
            if client._watching_task is not None:
                client._watching_task.cancel()
                client._watching_task = None
            if client._mnt_task is not None:
                client._mnt_task.cancel()
                client._mnt_task = None
            await client.websocket.stop(clear_topics=True)
            if client._session is not None:
                cookie_jar = cast(aiohttp.CookieJar, client._session.cookie_jar)
            ...
            client.wanted_games.clear()
            client._mnt_triggers.clear()
            await asyncio.sleep(start_time + 0.5 - time())

        client.shutdown = new_shutdown
    ...
    def start(self):
        ...
        # Start background tasks for clients 2-20
        # Client 1 is handled by the main asyncio loop in main.py calling client.run()
        for i, client in enumerate(self._clients):
            if i == 0: continue
            asyncio.create_task(self._run_client_bg(client))

    async def _run_client_bg(self, client: Twitch):
        try:
            await client._run()
        except Exception:
            pass
    ...
    def unfocus(self, event):
        ...
        self.settings_panel.clear_selection()

    def save(self, *, force: bool = False) -> None:
        self._cache.save(force=force)
        # Settings are shared, saved via User 1's reference
        self._clients[0].settings.save(force=force)
    ...
    # Legacy method support via Proxy (User 1)
    def display_drop(self, drop: TimedDrop, *, countdown: bool = True, subone: bool = False):
    ...
    def apply_theme(self, dark: bool) -> None:
        ...
        self.settings_panel._priority_list.configure_theme(bg=surface, fg=fg, sel_bg=sel_bg, sel_fg=sel_fg)
        self.settings_panel._exclude_list.configure_theme(bg=surface, fg=fg, sel_bg=sel_bg, sel_fg=sel_fg)
